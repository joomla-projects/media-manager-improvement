 ...
 * @since  3.0
 */
abstract class JHtmlMediaAdministrator
{
	/**
	...
	public static function association($articleid)
	{
		...
		if ($associations = JLanguageAssociations::getAssociations('com_media', '#__media_files', 'com_media.item', $articleid))
		{
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url     = JRoute::_('index.php?option=com_media&task=article.edit&id=' . (int) $item->id);
					...
					$item->link = '<a href="' . $url . '" title="' . $item->language_title . '" class="' . $classes
						. '" data-media="' . $tooltip . '" data-placement="top">'
						. $text . '</a>';
				}
			}
			...
			$html = JLayoutHelper::render('joomla.media.associations', $items);
		}
	...
	public static function featured($value = 0, $i, $canChange = true)
	{
		...
		$states = array(
			0 => array('unfeatured', 'articles.featured', 'COM_MEDIA_UNFEATURED', 'JGLOBAL_TOGGLE_FEATURED'),
			1 => array('featured', 'articles.unfeatured', 'COM_MEDIA_FEATURED', 'JGLOBAL_TOGGLE_FEATURED'),
		);
